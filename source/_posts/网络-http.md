...
#### http0.9 1.0 1.1的区别
HTTP（Hypertext Transfer Protocol）是用于在Web上进行通信的协议。以下是HTTP 0.9、1.0、1.1和2.0之间的主要区别：

HTTP 0.9：

首个版本的HTTP协议，只支持纯文本数据传输。
只有一个HTTP方法：GET。
不支持请求头部和响应头部，也没有状态码等其他元数据。
不支持持久连接，每次请求都需要建立新的TCP连接。
HTTP 1.0：

引入了请求头部和响应头部，使得更多的元数据可以在请求和响应中传输。
支持多种HTTP方法，如GET、POST、PUT等。
支持状态码，用于表示请求和响应的状态。
不支持持久连接，默认每个请求都使用一个新的TCP连接。
不支持管道化（pipelining），即在一个TCP连接上发送多个请求，而无需等待每个请求的响应。
HTTP 1.1：

引入了持久连接（persistent connections），即在单个TCP连接上可以发送多个请求和响应。
